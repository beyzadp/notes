...
| symmetric            | tied to (internal host, destination): the router picks a different external port for every destination    | impossible(?): the external port changes per destination so you cant predict it in advance (not even from a packet to a stun server), pure hole punching usually doesnt work, need a relay (turn) instead |
